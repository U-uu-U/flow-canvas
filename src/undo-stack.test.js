...
test('state timeline: reset/commit/undo/redo 保留完整画板快照', () => {
    const stack = new U.UndoStack(5);
    stack.resetState({
        activeGroupId: 'g1',
        items: items('a'),
        plans: [{ id: 'p1' }],
    ...
    assert.equal(stack.commitState({
        activeGroupId: 'g1',
        items: items('a', 'b'),
        plans: [{ id: 'p1' }],
        connections: [{ id: 'edge' }],
    ...
    const undone = stack.undoState();
    assert.deepStrictEqual(undone.items.map(item => item.id), ['a']);
    assert.deepStrictEqual(undone.viewport, { x: 0, y: 0, scale: 1 });
    const redone = stack.redoState();
    assert.deepStrictEqual(redone.items.map(item => item.id), ['a', 'b']);
    assert.deepStrictEqual(redone.connections, [{ id: 'edge' }]);
});

test('state timeline: 相同状态不重复入栈且运行态不参与比较', () => {
    const stack = new U.UndoStack();
    stack.resetState({ items: [{ id: 'a', runStatus: 'idle' }], connections: [] });
    assert.equal(stack.commitState({
        items: [{ id: 'a', runStatus: 'running', runError: 'temporary' }],
        connections: []
    }), false);
